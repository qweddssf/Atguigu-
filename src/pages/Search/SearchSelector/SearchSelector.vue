<template>
  <div class="clearfix selector">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <li
            ...
            :key="trademark.tmId"
            @click="getTradeM(trademark.tmName)"
          >
            {{ trademark.tmName }}
          </li>
    ...
    >
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(attv, idx) in att.attrValueList" :key="idx">
            <a>{{ attv }}</a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    getTradeM(tm) {
      this.$bus.$emit("getTradeMark", tm);
    },
    attrInfo(e) {
      if (e.target.tagName == "A") {
        const prop = e.target.innerText;
        this.$bus.$emit("getProps", prop);
      }
    },
  },
};
